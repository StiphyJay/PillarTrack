...
def decode_ry(pred_ry, head_bin_num, get_ry_fine):
    ry_bin_l, ry_bin_r = 0, head_bin_num
    ry_res_l, ry_res_r = ry_bin_r, ry_bin_r + head_bin_num

    ry_bin = torch.argmax(pred_ry[:, ry_bin_l: ry_bin_r], dim=1)
    ry_res_norm = torch.gather(pred_ry[:, ry_res_l: ry_res_r], dim=1, index=ry_bin.unsqueeze(dim=1)).squeeze(dim=1)
    if get_ry_fine:
        # divide pi/2 into several bins
        angle_per_class = (np.pi / 2) / head_bin_num
        ...
        angle_per_class = (2 * np.pi) / head_bin_num
        ry_res = ry_res_norm * (angle_per_class / 2)
...
def iou3d_torch(boxes_a, boxes_b):
    """
    Args:
        boxes_a: (1, 7) [x, y, z, dx, dy, dz, heading]
        boxes_b: (1, 7) [x, y, z, dx, dy, dz, heading]
    ...
    corners_a = boxes_to_corners_3d(boxes_a)[0]
    corners_b = boxes_to_corners_3d(boxes_b)[0]
    rect1 = [(corners_a[i,0], corners_a[i,1]) for i in range(3,-1,-1)]
    rect2 = [(corners_b[i,0], corners_b[i,1]) for i in range(3,-1,-1)] 
    area1 = poly_area(corners_a.new_tensor(rect1)[:,0], corners_a.new_tensor(rect1)[:,1])
    area2 = poly_area(corners_b.new_tensor(rect2)[:,0], corners_b.new_tensor(rect2)[:,1])
    ...
    overlaps_3d = inter_area * overlaps_h
...
def get_bev_box_mask(gt_box, height, width, pc_range, voxel_size):
    grid_x, grid_y = np.meshgrid(np.linspace(0, width-1, width),
    ...
    ct2d = (gt_box[:2]-pc_range[:2])/voxel_size[:2]
    w2d = (gt_box[3:4]/voxel_size[0]/2).item()
    l2d = (gt_box[4:5]/voxel_size[1]/2).item()
    corners = np.array([
        [-w2d, -l2d],
        [-w2d, l2d],
        [w2d, l2d],
    ...
    corners = np.matmul(corners[:, 0:2], rot_matrix)
    corners[:,:2] += ct2d[:2]
    path = Path(corners)
    mask = path.contains_points(img_grid)
    return mask