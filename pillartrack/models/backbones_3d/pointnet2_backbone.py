...
class PointNet2MSG(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        search_points, template_points = batch_dict['search_points'], batch_dict['template_points']
        
        template_point_feature, template_point_coords = self.forward_feature(batch_size, template_points)
        search_point_feature, search_point_coords = self.forward_feature(batch_size, search_points)
        pdb.set_trace()
        batch_dict['template_point_feature'] = template_point_feature
        batch_dict['template_point_coords'] = template_point_coords
        batch_dict['search_point_feature'] = search_point_feature
        batch_dict['search_point_coords'] = search_point_coords
        return batch_dict

    def forward_feature(self, batch_size, points):
        """
        Args:
            batch_dict:
                batch_size: int
        ...
        # BUG
        point_coords = torch.cat((batch_idx[:, None].float(), l_xyz[-1].view(-1, 3)), dim=1)
        return point_features.view(-1, point_features.shape[-1]), point_coords
...
class PointNet2Backbone(nn.Module):
    ...
    def forward_feature(self, batch_size, points):
        ...
        point_coords = torch.cat((batch_idx[:, None].float(), l_xyz[0]), dim=1)
